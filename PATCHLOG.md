...
- 퀘이사존 게시판 HTML 구조 변경으로 게시글을 수집하지 못하는 오류 수정
- 새 구조의 제목, 작성자, 추천수, 조회수, 가격, 배송비 및 종료 상태 파싱 지원
- 기존 게시판 구조 호환성 유지 및 회귀 테스트 추가

## v2.3.1 (2026-07-25)
- 알려진 취약점이 수정된 버전으로 의존성 업데이트
  - `aiohttp` 3.14.3
  - `soupsieve` 2.9.1
  - `protobuf` 7.35.1
  - `urllib3` 2.7.0
- 그 외 직접 및 전이 의존성 업데이트

## v2.3.0 (2026-07-25)
- HTTP 클라이언트 구조 개선
  - `HttpClient` 프로토콜을 추가하고 크롤러가 구현체를 주입받도록 변경
  - `curl_cffi` 기반 클라이언트를 기본 구현으로 추가
  - `aiohttp` 기반 클라이언트 유지
  - Cloudflare DNS와 DoH를 사용하도록 요청 경로 변경
- 크롤러 파싱 오류 수정
  - 뽐뿌 작성자 및 RSS 조회 정보
  - 다모앙 게시판 목록
  - 에펨코리아 게시글 제목
  - ZOD 특가 정보
- 쿨앤조이 진단 CLI가 RSS 파서를 사용하도록 수정
- HTTP 클라이언트 및 크롤러 회귀 테스트 추가
